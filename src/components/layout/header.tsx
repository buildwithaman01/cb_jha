...
// Force rebuild to clear stale SSR cache - attempting to bypass Turbopack cache bug
      ...
      >
        <Container>
          <div className="flex h-16 md:h-18 items-center justify-between">
            {/* Logo */}
            <Link href="/" className="relative flex items-center shrink-0 w-[240px] h-[55px] md:w-[320px] md:h-[72px]" aria-label="C.B. Jha Tutorials Home">
              <Image
                src="/images/logo.png"
            ...
            <div className="hidden lg:flex items-center gap-3">
              <a
                ...
                className={cn(
                  ...
                  isHome && !scrolled ? "text-white/70 hover:text-white" : "text-foreground/70 hover:text-foreground"
                )}
              >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[60] bg-[#0A0E14] flex flex-col"
          ...
          >
            ...
            {/* Nav Links */}
            <nav className="flex flex-col flex-1 px-6 py-8 overflow-y-auto" aria-label="Mobile navigation">
              {NAV_LINKS.map((link, i) => (
                <motion.div
                  key={link.href}
                  initial={{ opacity: 0, x: -24 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: i * 0.06 + 0.05, ease: "easeOut" }}
                >
                  <Link
                    href={link.href}
                    onClick={() => setIsOpen(false)}
            ...
            <motion.div
              ...
              className="px-6 pb-6 pt-4 border-t border-white/8 space-y-3"
            >
              <Link
                href="/contact"
                onClick={() => setIsOpen(false)}
